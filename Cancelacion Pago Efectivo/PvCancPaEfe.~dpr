...
begin
  ...
  IF  TNOMINA='O' THEN begin  tppagos:='PPAGOS'; TPDPAGOS:='PDPAGOS';  end;
  IF  TNOMINA='H' THEN begin  tppagos:='PHPAGOS'; TPDPAGOS:='PHDPAGOS';  end;
  IF  TNOMINA='N' THEN begin  tppagos:='PNPAGOS'; TPDPAGOS:='PNDPAGOS';  end;
  ...
  Q.close;
  q.sql.text:='SELECT * FROM '+TPPAGOS+' WHERE PAGO_PAGO='+NUMERO;
  Q.open;

  s.close;
  s.sql.text:='SELECT PUES_DESCRIP FROM PPUESTOS WHERE PUES_PSTO='+#39+q.fieldbyname('PAGO_PSTO').asstring+#39;
  S.OPEN;
  ...
  S.close;
  S.sql.text:='SELECT sum(dpag_monto) as percep FROM '+tPDPAGOS+' WHERE DPAG_PAGO='+#39+NUMERO+#39+
  ...
  S.close;
  S.sql.text:='SELECT sum(dpag_monto) as deduc FROM '+tPDPAGOS+' WHERE DPAG_PAGO='+#39+NUMERO+#39+
