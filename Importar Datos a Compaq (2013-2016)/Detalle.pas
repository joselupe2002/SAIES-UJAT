...
IF (tiponom.Text<>'') AND (TIPOPER.TEXT<>'') AND (PERIODOCQ.TEXT<>'') then
   begin
      ...
      SQL:='SELECT DISTINCT(VDPA_EMPL) AS IDEMPLEADO, 1 AS IDDEPARTAMENTO, '+npsto.caption+' AS IDPUESTO,'+
           ...
           #39+'S'+#39+' AS TIPOEMPLEADO, getFormaPago(vdpa_empl'+','+#39+periodo.text+'/'+anio.text+#39+') AS FORMAPAGO,'+
           #39+'E'+#39+' AS BASEPAGO,'+
...
{              cq.close;
              cq.sql.text:='UPDATE nom10034 SET formapago= (select a.formapago from nom10001 a where a.idempleado=nom10034.idempleado)'+
              ' where cidperiodo='+PCQ.text;
               savetofilelog(cq.sql.text);
              cQ.EXECSQL;
