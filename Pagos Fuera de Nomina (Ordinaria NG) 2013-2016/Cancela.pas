...
uses
  ...
  DBGrids, StdCtrls, DBCtrls, Mask, Buttons,OleServer, Excel97, Comobj;
...
begin
  ...
  ' S.DPAG_MONTO AS MONTO, DPAG_CNTA AS CUENTA, DPAG_SCTA AS SUBCUENTA FROM PMOVFNPDPAGOSR S WHERE S.DPAG_PAGO='+#39+PAGO.TEXT+#39+
  ...
  ' SUM(DPAG_MONTO) AS MONTO FROM PMOVFNPDPAGOSR S WHERE S.DPAG_PAGO='+#39+PAGO.TEXT+#39+
  ...
  Q.CLOSE;
  Q.SQL.TEXT:='SELECT SUM(DPAG_MONTO)  FROM PMOVFNPDPAGOSR S WHERE S.DPAG_PAGO='+#39+PAGO.TEXT+#39+
  ' AND S.DPAG_PERDED='+#39+'P'+#39;
  Q.open;
...
begin
  IF FPAGO.ItemIndex>=0 THEN
     begin
        q.close;
        q.SQL.text:='CALL CANCELAPAGOFN('+#39+PAGO.TEXT+#39+','+
...
if Application.MessageBox('Seguro que desea Cancelar el Pago Fuera de Nomina','Confirmar',
         ...
         begin
             q.Close;
             q.sql.text:='UPDATE PMOVFUENOM SET '+
             'MOVF_CANCEL='+#39+'S'+#39+','+
...
procedure TFCancela.TransChange(Sender: TObject);
begin
if trans.Text='S' then
   fpago.ItemIndex:=0
else
   fpago.ItemIndex:=1
