...
q.DataBaseName := 'Sistema' ;
 if xmodulo.Query1.FieldByName('VSOL_ENVIADASIN').AsString ='N' then
       if (Application.MessageBox('Seguro que desea Enviar la solicitud a Sindicato', 'Confirmar', MB_ICONQUESTION+ MB_YESNO)= IDYES) then
          begin
             Q.CLOSE;
...
begin
 ...
 case xmodulo.modo of
  ...
  2:  begin if xmodulo.Query1.FieldByName('VSOL_ENVIADASIN').AsString='N' Then
         result := TFDetalle.Create(Xmodulo)
      else
         Showmessage('Las solicitudes que ya fueron enviadas al Sindicato no pueden ser modificadas');
      end;
  4 : result:=nil;
 end ;
end ;
