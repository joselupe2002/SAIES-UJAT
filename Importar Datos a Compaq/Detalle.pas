...
uses
  ...
  TFDetalle = class(TPFDetalle)
    ...
    procedure MigrarPuestosClick(Sender: TObject);
...
begin
  inherited;
  //Migramos los puestos que su naturaleza no es la correcta y se graban en CQPPUESTOS
  q.Close;
  q.sql.text:='call CQCreaConcNew('+#39+periOdo.text+'/'+ANIO.text+#39+')';
  obsp.Lines.Add('Creando y actualizando puestos ....');
  q.Execsql;
  ...
  Q.Close;
  q.sql.text:='SELECT SQLCOD, TABLACQ FROM CQGENERAL WHERE CONSULTA='+#39+RUBRO+#39;
  savetofilelog(q.sql.text);
  Q.open;

  SQL:=Q.fields[0].asstring;
  tablacq:=Q.fields[1].asstring;
  ...
  while not(q.eof) do
     begin
         ...
         CQ.close;
         cq.sql.text:='SELECT COUNT(*) FROM '+tablacq+' WHERE NUMEROCONCEPTO='+#39+inttostr(Q.Fields[0].asinteger+200)+#39;
         ...
         if cq.fields[0].asinteger<=0 then //No existe el registro se inserta
            begin
                cq.close;
                cq.sql.text:='INSERT INTO '+TABLACQ+'(numeroconcepto, tipoconcepto, descripcion, '+
                ...
                #39+inttostr(Q.fields[0].asinteger+200)+#39+','+#39+Q.fields[1].asstring+#39+','+#39+Q.fields[2].asstring+#39+','+
                '0,0,0,1,0,'+#39+''+#39+','+
                #39+''+#39+','+#39+''+#39+','+#39+''+#39+','+
                ...
                SAVETOFILELOG(CQ.SQL.TEXT);
                Cq.execsql;
                obsp.Lines.Add('Se inserto :'+q.fields[0].asstring);
            end
         else
             begin
                 obsp.Lines.Add('Ya existia en Tabla :'+q.fields[0].asstring);
                 if actualiza.checked then
                     begin
                        cq.close;
                        cq.sql.text:='UPDATE '+TABLACQ+' SET '+
                        'tipoconcepto='+#39+Q.fields[1].asstring+#39+','+
                        'descripcion='+#39+Q.fields[2].asstring+#39+','+
                        'claveagrupadorasat='+#39+Q.fields[3].asstring+#39+
                        ' Where numeroconcepto='+inttostr(Q.fields[0].asinteger+200);
                         Cq.execsql;
                         obsp.Lines.Add('Se actualizo el registro :'+q.fields[0].asstring);
                     end;
             end;
     ...
     aux.close;
     aux.sql.text:='DELETE FROM CQRELTAB WHERE TABLA='#39+RUBRO+#39;
     Aux.ExecSQL;
     q.close;
     q.sql.text:=sql;
     q.Open;
     ...
     while not(q.eof) do
        begin
           //Verificamos que no exista el registro
           CQ.close;
           cq.sql.text:='SELECT idconcepto FROM '+tablacq+' WHERE NUMEROCONCEPTO='+inttostr(Q.Fields[0].asinteger+200);
           cq.Open;
           aux.close;
           ...
           #39+RUBRO+#39+')';
           Aux.ExecSQL;
           obsp.Lines.Add('Se inserto relación :'+q.fields[0].asstring+'--'+cq.fields[0].asstring);
            barrap.Progress:=barraP.Progress+1;
           Q.next;
        end;
end;
...
begin
  ...
  while not(q.eof) do
     begin
         //Verificamos que no exista el registro
         CQ.close;
         cq.sql.text:='SELECT COUNT(*) FROM '+tablacq+' WHERE NUMERODEPARTAMENTO='+Q.Fields[0].asstring;
         savetofilelog(Cq.sql.text);
         cq.Open;
         ...
         if cq.fields[0].asinteger<=0 then //No existe el registro se inserta
            begin
                cq.close;
                cq.sql.text:='INSERT INTO '+TABLACQ+'(numerodepartamento, descripcion, '+
                'beneficiario, cuentacw, timestamp, csegmentonegocio) VALUES ('+
                #39+Q.fields[0].asstring+#39+','+#39+Q.fields[1].asstring+#39+','+
                #39+''+#39+','+#39+''+#39+','+#39+DATETOSTR(DATE)+#39+','+#39+''+#39+')';
                Cq.execsql;
                obsp.Lines.Add('Se inserto :'+Rubro+' '+q.fields[0].asstring);
            end
         else
             begin
                 obsp.Lines.Add('Ya existia en Tabla :'+Rubro+' '+q.fields[0].asstring);
                 IF ACTUALIZA.Checked then
                    begin
                       cq.close;
                       cq.sql.text:='UPDATE '+TABLACQ+' SET '+
                       ' descripcion='+ #39+Q.fields[1].asstring+#39+
                       ' WHERE numerodepartamento='+ #39+Q.fields[0].asstring+#39;
                       Cq.execsql;
                       obsp.Lines.Add('Se Actualizo :'+Rubro+' '+q.fields[0].asstring);
                     end;
     ...
     while not(q.eof) do
        begin
           //Verificamos que no exista el registro
           CQ.close;
           cq.sql.text:='SELECT IDDEPARTAMENTO FROM '+tablacq+' WHERE numerodepartamento='+Q.Fields[0].asString;
           cq.Open;
           aux.close;
           aux.sql.text:='INSERT INTO CQRELTAB (SAIES,COMPAQ,TABLA) VALUES ('+
           #39+Q.Fields[0].asString+#39+','+
           ...
           #39+RUBRO+#39+')';
           Aux.ExecSQL;
           obsp.Lines.Add('Se inserto relación :'+Rubro+' '+q.fields[0].asstring+'--'+cq.fields[0].asstring);
            barrap.Progress:=barraP.Progress+1;
           Q.next;
        end;
end;
...
IF (tiponom.Text<>'') AND (TIPOPER.TEXT<>'') AND (PERIODOCQ.TEXT<>'') then
   begin
      ...
      SQL:='SELECT DISTINCT(VDPA_EMPL) AS IDEMPLEADO, 1 AS IDDEPARTAMENTO, '+npsto.caption+' AS IDPUESTO,'+
           STR_(TIPOPER.TEXT,'-')+' AS IDTIPOPERIODO, '+nturno.caption+' AS IDTURNO, S.VDPA_Empl AS CODIGOEMPLEADO, SUBSTR(U.PERS_NOMBRE,1,40) AS NOMBRE,'+
           ...
           'V.ATRE_FNACI AS FECHANACIMIENTO,'+
           ...
           'SUBSTR(U.PERS_RFC,11,5) AS HOMOCLAVE,'+#39+'0'+#39+' AS CUENTAPAGOELECTRONICO, '+
            #39+'0'+#39+' AS SUCURSALPAGOELECTRONICO,'+#39+'A'+#39+' AS ESTADOEMPLEADO, 100 AS SUELDODIARIO,'+
           ...
           #39+'S'+#39+' AS TIPOEMPLEADO,'+#39+'E'+#39+' AS FORMAPAGO,'+#39+'E'+#39+' AS BASEPAGO,'+
          ...
          'V.ATRE_EMAIL AS CORREOELECTRONICO, CQFTIPOREGI('+'VDPA_EMPL'+','+#39+periodo.text+'/'+anio.text+#39+') AS TIPOREGIMEN, '+#39+'000000000000000000'+#39+' AS CLABEINTERBANCARIA'+
          ' FROM CQPVDPAGOS S,  FPERSONAS U, CQVPATREMPL V  WHERE S.VDPA_PERIODO='+#39+periodo.text+'/'+anio.text+#39+
          ' AND S.VDPA_Empl=U.PERS_PERSONA '+
          ' AND S.VDPA_Empl=V.ATRE_EMPL(+)'+
      ...
      while not(q.eof) do
         begin
             ...
             if cq.fields[0].asinteger<=0 then //No existe el registro se inserta
                begin
                    ...
                    cq.sql.text:='INSERT INTO nom10001 (iddepartamento, idpuesto, idtipoperiodo,'+
                                 ...
                                 'TipoRegimen, ClabeInterbancaria) VALUES ( '+
                                  ...
                                  #39+'1'{Q.fieldbyname('IDPUESTO').asstring}+#39+','+
                    ...
                    'B.ccampoextranumerico3, B.ccampoextranumerico4, B.ccampoextranumerico5, 0, 0,'+
                    '0, 0, 0, 0, 0, 0,'+
              ...
              if str_(tipoNom.text,'-')='HON' THEN
                begin
                   cq.close;
                   cq.sql.text:='UPDATE nom10001 SET tiporegimen=2';
                   cQ.EXECSQL;
                end;
              if str_(tipoNom.text,'-')='H' THEN
                begin
                   cq.close;
                   cq.sql.text:='UPDATE nom10001 SET tiporegimen=9';
                   cQ.EXECSQL;
                end;

              if str_(tiponom.text,'-')='J' then
                 begin
                   cq.close;
                   cq.sql.text:='UPDATE nom10001 SET tiporegimen=3';
                   cQ.EXECSQL;
                 end;
            ...
            {  cq.close;
              cq.sql.text:='UPDATE nom10001 SET fechaalta='+#39+Q.fieldbyname('fechaalta').asstring+#39+','+
              'fechaSueldoDiario='+#39+Q.fieldbyname('fechaalta').asstring+#39+',fechaSueldoVariable='+
               #39+Q.fieldbyname('fechaalta').asstring+#39+
              ',fechaSueldoPromedio='+#39+Q.fieldbyname('fechaalta').asstring+#39+','+
              'fechaSueldoIntegrado='+#39+Q.fieldbyname('fechaalta').asstring+#39+', IDDEPARTAMENTO=1, IDPUESTO=1, idturno=1 ';
              cQ.EXECSQL;
              //
              cq.close;
              cq.sql.text:='UPDATE nom10034 SET fechaalta='+#39+Q.fieldbyname('fechaalta').asstring+#39+','+
              'fechaSueldoDiario='+#39+Q.fieldbyname('fechaalta').asstring+#39+','+
              'fechaSueldoVariable='+#39+Q.fieldbyname('fechaalta').asstring+#39+',fechaSueldoPromedio='+
              #39+Q.fieldbyname('fechaalta').asstring+#39+','+
              'fechaSueldoIntegrado='+#39+Q.fieldbyname('fechaalta').asstring+#39+', cestadoempleadoPERIODO='+#39+'A_'+#39;
              cq.execsql;   }


             {//Actualizamos
              cq.close;
              cq.sql.text:='UPDATE nom10001 SET fechaalta='+#39+datetostr(now)+#39+','+
              'fechaSueldoDiario='+#39+datetostr(now)+#39+',fechaSueldoVariable='+
               #39+datetostr(now)+#39+
              ',fechaSueldoPromedio='+#39+datetostr(now)+#39+','+
              'fechaSueldoIntegrado='+#39+datetostr(now)+#39+', IDDEPARTAMENTO=1, IDPUESTO=1, idturno=1 ';
              cQ.EXECSQL;
              //
              cq.close;
              cq.sql.text:='UPDATE nom10034 SET fechaalta='+#39+datetostr(now)+#39+','+
              'fechaSueldoDiario='+#39+datetostr(now)+#39+','+
              'fechaSueldoVariable='+#39+datetostr(now)+#39+',fechaSueldoPromedio='+
              #39+datetostr(now)+#39+','+
              'fechaSueldoIntegrado='+#39+datetostr(now)+#39+', cestadoempleadoPERIODO='+#39+'A_'+#39;
              cq.execsql;    }
         ...
         while not(q.eof) do
            begin
               ...
               #39+'FPERSONAS'+#39+')';
               TRY Aux.ExecSQL;
               except  on E: Exception  do begin obsError.Lines.Add('Error Insertando'+q.fields[1].asstring+' '+E.Message);end; end;
               obsp.Lines.Add('Se inserto relación Empleados --> '+q.fields[0].asstring+'--'+cq.fields[0].asstring);
                barrap.Progress:=barraP.Progress+1;
               Q.next;
            end;
...
begin
  ...
  while not(q.eof) do
     begin
         //Verificamos que no exista el registro
         CQ.close;
         cq.sql.text:='SELECT COUNT(*) FROM '+tablacq+' WHERE NUMEROPUESTO='+Q.Fieldbyname('NUMERO').asstring;
         savetofilelog(Cq.sql.text);
         cq.Open;
         ...
         if cq.fields[0].asinteger<=0 then //No existe el registro se inserta
            begin
                cq.close;
                cq.sql.text:='INSERT INTO '+TABLACQ+'(numeropuesto, descripcion, timestamp) VALUES ('+
                #39+Q.Fieldbyname('NUMERO').asstring+#39+','+#39+Q.fields[1].asstring+#39+
                ','+#39+datetostr(time)+#39+')';
                savetofilelog(Cq.sql.text);
                Cq.execsql;
                obsp.Lines.Add('Se inserto :'+Rubro+' '+q.fields[0].asstring+'-'+Q.Fieldbyname('NUMERO').asstring);
            end
         else
             begin
                 obsp.Lines.Add('Ya existia en Tabla :'+Rubro+' '+q.fields[0].asstring+'-'+Q.Fieldbyname('NUMERO').asstring);
                 if actualiza.Checked then
                    begin
                       cq.close;
                       cq.sql.text:='UPDATE '+TABLACQ+' SET '+
                       ' descripcion='+ #39+Q.fields[1].asstring+#39+','+
                       ' TIMESTAMP='+ #39+datetostr(date)+#39+
                       ' WHERE numeropuesto='+ #39+Q.Fieldbyname('NUMERO').asstring+#39;
                       savetofilelog(Cq.sql.text);
                       Cq.execsql;
                       obsp.Lines.Add('Se Actualizo :'+Rubro+' '+q.fields[0].asstring+'-'+Q.Fieldbyname('NUMERO').asstring);
                   end;
             end;
         barrap.Progress:=barraP.Progress+1;
         q.next;
     ...
     while not(q.eof) do
        begin
           //Verificamos que no exista el registro
           CQ.close;
           cq.sql.text:='SELECT IDPUESTO FROM '+tablacq+' WHERE numeroPuesto='+Q.Fieldbyname('NUMERO').asstring;
           cq.Open;
           aux.close;
           aux.sql.text:='INSERT INTO CQRELTAB (SAIES,COMPAQ,TABLA) VALUES ('+
           #39+Q.Fields[0].asString+#39+','+
...
BEGIN
  q.close;
  Q.sql.text:='SELECT SUM(A.VDPA_MONTO) FROM CQPVDPAGOS A WHERE A.VDPA_PERDED='+#39+'P'+#39+
  ...
  q.close;
  Q.sql.text:='SELECT SUM(A.VDPA_MONTO) FROM CQPVDPAGOS A WHERE A.VDPA_PERDED='+#39+'D'+#39+
  ' AND A.VDPA_PERIODO='+#39+PERIODO.text+'/'+anio.text+#39+
  ' AND A.VDPA_EMPL NOT LIKE '+#39+'DJ%'+#39+
  ' AND A.VDPA_QUENOM IN ('+QUENOM+')';
  q.open;
  ...
  //Por fondos
  q.close;
  Q.sql.text:='SELECT a.vdpa_sfdo, SUM(A.VDPA_MONTO) FROM CQPVDPAGOS A WHERE A.VDPA_PERDED='+#39+'P'+#39+
  ' AND A.VDPA_PERIODO='+#39+PERIODO.text+'/'+anio.text+#39 +
  ' AND A.VDPA_EMPL NOT LIKE '+#39+'DJ%'+#39+
  ...
  q.close;
  Q.sql.text:='SELECT a.vdpa_sfdo, SUM(A.VDPA_MONTO) FROM CQPVDPAGOS A WHERE A.VDPA_PERDED='+#39+'D'+#39+
  ' AND A.VDPA_PERIODO='+#39+PERIODO.text+'/'+anio.text+#39+
  ' AND A.VDPA_EMPL NOT LIKE '+#39+'DJ%'+#39+
  ' AND A.VDPA_QUENOM IN ('+QUENOM+') group by a.vdpa_sfdo';
  q.open;
...
begin
  inherited;
if Application.MessageBox('¿Seguro que desea correr el proceso de juntar nóminas se perderá historico?','Confirmar',MB_ICONQUESTION+ MB_YESNO)= IDYES THEN
   begin
      IF tiponom.Text<>'' then
      BEGIN
        if periodofin.text<>'' then
           begin
              ...
              obsp.Lines.Add('Cambinando conceptos de pagos HAS....');
              q.Execsql;
...
procedure TFDetalle.MigrarPuestosClick(Sender: TObject);
begin
  inherited;
{  q.Close;
  q.sql.text:='call CQCreaPstoNew(to_date('+#39+periodo.text+'/'+anio.text+#39'))';
  q.execsql;     }

  //Creamos el Puesto por default
  cq.close;
  cq.sql.text:='SELECT IDPUESTO FROM nom10006 WHERE numeropuesto='+#39+'0'+#39;
  cq.open;
  if (cq.Eof) then
     begin
        CQ.close;
        cq.sql.text:='INSERT INTO nom10006(NUMEROPUESTO, DESCRIPCION) VALUES ('+#39+'0'+#39+','+#39+'NINGUNO'+#39+')';
        cq.execsql;
        cq.close;
        cq.sql.text:='SELECT IDPUESTO FROM nom10006 WHERE numeropuesto='+#39+'0'+#39;
        cq.Open;
        npsto.Caption:=cq.Fields[0].asstring;
     end
  else
      npsto.Caption:=cq.Fields[0].asstring;

  //Creamos el turno por default
  cq.close;
  cq.sql.text:='SELECT IDTURNO FROM nom10032 WHERE NUMEROTURNO='+#39+'5'+#39;
  cq.open;
  if (cq.Eof) then
     begin
        CQ.close;
        cq.sql.text:='INSERT INTO nom10032(numeroturno, DESCRIPCION, numerohoras) '+
        ' VALUES ('+#39+'5'+#39+','+#39+''+#39+','+#39+'8'+#39+')';
        cq.execsql;
        cq.close;
        cq.sql.text:='SELECT idturno FROM nom10032 WHERE NUMEROTURNO='+#39+'5'+#39;
        cq.Open;
        nturno.Caption:=cq.Fields[0].asstring;
     end
  else
      nturno.Caption:=cq.Fields[0].asstring;
  Showmessage('Se creo puesto por Default y Turno para el periodo '+periodo.text+'/'+anio.text);
...
begin
  inherited;
  cqTablas.close;
  cqTablas.sql.text:='SELECT CODIGOEMPLEADO FROM nom10001';
  CQTablas.OPEN;
  while not(CQTablas.eof) do
     begin
         q.close;
         q.sql.text:='select V.ATRE_EMAIL AS CORREOELECTRONICO, SUBSTR(V.ATRE_CURP,1,4) AS CURPI, '+
           ' SUBSTR(V.ATRE_CURP,11,10) AS CURPF,'+
           ' nvl(V.ATRE_NUMSS,'+#39+'99999999999'+#39+') AS NUMEROSEGUROSOCIAL,'+#39+'0'+#39+' AS UMF, '+
           ' SUBSTR(S.PERS_RFC,1,4) AS RFC,SUBSTR(S.PERS_RFC,11,5) AS HOMOCLAVE,'+#39+'0'+#39 +
           ' from fpersonas s, CQVPATREMPL v where s.pers_persona=v.ATRE_EMPL '+
           ' and s.pers_persona='+#39+CQTablas.fields[0].asstring+#39;
         q.Open;
         ...
         cq.sql.text:='UPDATE nom10001 SET '+
                     ...
                     'RFC='+#39+COPY(Q.fieldbyname('RFC').asstring,1,4)+#39+','+
                     'HOMOCLAVE='+#39+COPY(Q.fieldbyname('HOMOCLAVE').asstring,1,4)+#39+','+
                     'CORREOELECTRONICO='+#39+TRIM(COPY(Q.fieldbyname('CorreoElectronico').asstring,1,60))+#39+','+
                     'numerosegurosocial='+#39+COPY(Q.fieldbyname('numerosegurosocial').asstring,1,11)+#39+
                     ' WHERE codigoempleado='+#39+CQTablas.fields[0].asstring+#39;
         savetofilelog(cq.sql.text);
         cq.EXECSQL;
         obsp.Lines.Add('Empleado actualizado-->'+cqTablas.fields[0].asstring);
