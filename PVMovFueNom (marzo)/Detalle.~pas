...
begin
 ...
 IF RESULT='PROYFN' THEN
     begin
       q.Close;
       q.sql.text:='select s.ures_pago from fures2013 s where s.ures_ures='+#39+uresfn.Text+#39;
       Q.open;
                ...
                begin
                     IF jubilado.text='N' then
                        begin
                            ...
                            else  //lOS DEMAS TIPOS DE EMPLEADOS Y NO SON JUBILADOS 
                               q.sql.text:='SELECT * FROM PMATREL WHERE CVE_CONCEPTO='+#39+str_(GRID.cells[newcol,ling],'-')+#39+
                               ' AND (TIPO_PLAZA IS NULL OR TIPO_PLAZA='+#39+TIPOPLAZA.TEXT+#39+') '+
...
begin
  inherited;
  FECINI.FIELD.ASDATEtime:=STRTODATE('01/01/2013');
    FECFIN.FIELD.ASDATEtime:=STRTODATE('02/01/2013');
   SFONDO.FIELD.AsString:='1101';
Grid.RowCount:=grid.RowCount+1;
GRID.CELLS[0,1]:='150';
COLG:=0;
getDescripGrid;
GRID.CELLS[3,1]:='P';
GRID.CELLS[1,1]:='3994.09';
DESCRIP.FIELD.ASSTRING:='PAGO DE ESTIMULO A MEJORES TRABAJADORES STAIUJAT.';
SOLICITO.FIELD.ASSTRING:='M.A. RUBICEL CRUZ ROMERO';
TEXTO.FIELD.ASSTRING:='PAGO DE ESTIMULO A MEJORES TRABAJADORES STAIUJAT.';
...
begin
  inherited;
  qdj.close;
  qdj.sql.text:='SELECT MOVF_PERS FROM PMOVFUENOM T WHERE T.MOVF_PADRE = (SELECT MAX(S.MOVF_SEQ) '+
              ' FROM PMOVFUENOM S WHERE S.MOVF_DESCRIP LIKE '+#39+'%LOGISTICA%'#39+
              ' AND S.MOVF_PADRE=0)';
  qdj.open;
