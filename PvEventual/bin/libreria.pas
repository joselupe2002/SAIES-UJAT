...
procedure numpoli(polid : Word ; libro : Char) ;
function PeriAb(Anio : Word ; mes : String) : Boolean ;
//
...
begin
 ...
 if libro= 'C' then
  begin
   ...
   mQuery.Sql.Add( 'POLI_TIPDOC As TIPDOC,');
   mQuery.Sql.Add( 'POLI_Anio As Anio,');
   mQuery.Sql.Add( 'POLI_Mes As Mes');
   mQuery.Sql.Add('From FPolizas Where Poli_Polid= '+ IntToStr(Polid) );
  end
 else
  begin
   mQuery.Sql.Add('SELECT');
   mQuery.Sql.Add( 'POLR_TIPDOC As TIPDOC,');
   mQuery.Sql.Add( 'POLR_Anio As Anio,');
   mQuery.Sql.Add( 'POLR_Mes As Mes');
   mQuery.Sql.Add('From FPolRvdo Where Polr_Polrd= '+ IntToStr(Polid) );
  end ;
 ...
 FCntPoli := TTable.Create(nil) ;
 FCntPoli.DatabaseName := 'Sistema' ;
 FCntPoli.Exclusive := True ;
 FCntPoli.TableName := 'FCNTPOLI' ;
 FCNTPOLI.Filter :=
   'CNTP_TIPDOC = '''+ Doc+ '''' +
   ' And CNTP_ANIO = ' + IntToStr(ano)+
   ' And CNTP_MES  = ''' + mes  + '''' ;
 FCNTPOLI.Filtered := True ;
 FCNTPOLI.Open ;
 num := FCNTPOLI['CNTP_Consec'] ;
 FCNTPOLI.Edit ;
 FCNTPOLI['CNTP_CONSEC'] := FCNTPOLI['CNTP_CONSEC'] + 1;
 FCNTPOLI.Post ;
 FCNTPOLI.Close ;
 ...
 if libro= 'R'
 then mQuery.Sql.Add
  ('Update FPOLRVDO Set PolR_Num= :Num Where Polr_Polrd= :Polid')
 else mQuery.Sql.Add
  ('Update FPOLIZAS Set Poli_Num= :Num Where Poli_Polid= :Polid') ;
 mQuery.ParamByName('Polid').AsWord := Polid ;
 mQuery.ParamByName('Num').AsWord := Num ;
 mQuery.ExecSql ;
 mQuery.Free ;
 FCntPoli.Free ;
end ;
...
begin
 mQuery := TQuery.Create(nil) ;
 mQuery.DataBaseName := 'Sistema' ;
 mQuery.Sql.Add('Select Peri_Status As Status From FPeriods ');
 mQuery.Sql.Add('Where Peri_Anio= :Anio And Peri_Mes= :Mes') ;
 mQuery.ParamByName('Anio').AsWord := Anio ;
 mQuery.ParamByName('Mes').AsString := mes ;
 mQuery.Open ;
 result := mQuery.FieldByName('Status').AsString= 'A' ;
 mquery.Close ;
 mQuery.Free ;
end ;
